...
static void instrument_memory_write(instr_t *instr, dr_mcontext_t *mc) {
  size_t size;
  opnd_t ref;
...
static void process_instr(app_pc instr_addr, platform_int_t offset) {
  char *cf_bit = "cf=0";
  char *pf_bit = "pf=0";
  char *af_bit = "af=0";
  char *zf_bit = "zf=0";
...
#elif defined(X86_32)
  fprintf(logfile, "[%p]:off=%d %03X - %-6s ", instr_addr, offset, opcode,
          opcode_name);
  if (instr_writes_memory(&instr)) {
    instrument_memory_write(&instr, &mc);
  }
...
#endif
    dr_insert_clean_call(drcontext, bb, instr, (void *)process_instr, false, 2,
